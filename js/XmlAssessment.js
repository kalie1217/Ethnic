  ...
  //method call when next button is clicked
	...
	function checkAnswer(){
        ...
        $(objRightWrong).parent().siblings('.tdQuestion').css('color', 'red');
  	    ...
  	    objTempTrue.onclick = null; //disable True button
  		  objTempTrue.style.cursor = 'default';
  		  objTempFalse.onclick = null; //disable False button
  		  objTempFalse.style.cursor = 'default';
  		  $(xid(rightWrong)).fadeTo('fast', 1);
  		  $(xid(i)).fadeIn('fast'); // display "why" button;
	    } //end for loop
    ...
    $("#scorePercent").show();
		...
		if(scorePercent > passingScore){
		 	if(certificate == "true"){//if final assesment
        if(version != 'scorm'){
...
function nextChap(){
	...
	function retake() {
		...
		var chapterLink = {href:'javascript:void(0);', onclick:currentChapter};
		$('.retake').remove();
		...
		if(certificate == "true"){ //this is the final assessment
			...
			if (version == 'scorm')
				retakeText += '<p>Please click on the &quot;Profile&quot; chapter and begin again.</p><p>Read each chapter carefully before attempting to retake the assessment.</p></div>';
			else
  	...
  	for(var i=0; i<xmlNode.childNodes.length; i++){
        ...
        if( xPart.nodeName == "" || xPart.nodeName == null || xPart.nodeName == 'null'){
          string += "";
  				...
  				string +=  "<" + xPart.nodeName + ">"; //html tags (b, u, i etc.)
  				if(xPart.firstChild){
