...
function randomNumbers () {
  ...
  this.fill = function (n) {
    this.ff = []
    for (var i = 0; i < n; i++) { this.ff[i] = true }
    this.count = n
  }
  this.get = function () {
    var n, k, r
  ...
  if (arguments.length > 0) { this.fill(arguments[0]) }
}// randomNumbers
...
function showMainBody (file) {
  $('#div_mainBody').load(file, function () {
    if (typeof load_afg === 'function') { load_afg() } // loading initialization for CO
  })
}
...
var validateOnEnter = function (event) {
  if (event.keyCode == 32 || event.keyCode == 39 || event.keyCode == 34) { event.returnValue = false }
...
function cookie_class () {
  ...
  function init () {
    ...
    if (r) {
      ...
      var a = v.split('&')
      for (var i = 0, len = a.length; i < len; i++) { a[i] = a[i].split(':') }
      for (var i = 0, len = a.length; i < len; i++) { _this[a[i][0]] = unescape(a[i][1]) }
    }
  }
  init()
  this.clear = function () {
...
function getURL_Parameter (param) {
  ...
  if (typeof urlQuery === 'undefined') { return 'undefined' }
