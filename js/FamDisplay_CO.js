...
function FamDisplay(){
	...
	var langCode;
	var chapTitles;
	var pageTotals;      //array of chapter page totals
	var cPageSpans;
	var tPageSpans;
	var chapInfoSpans;
	var upBtns;
  ...
  var language;
	...
	this.displayDropDown = function(ddMenu){ //title, drop down, chapter name/number
	...
	function updateDropDown(){
		...
		if(ddm.nodeName == "SELECT"){
			if(ddm.nodeName == "SELECT"){
				var pi  = xc_obj.getPageIndex();      //page index
				var mi = ddm.options.length-1;        //menu index
	...
	function updateNumberPages(){
	    ...
	    var total = 0;
		  if(version == 'famWeb'){ //not scorm total page of previous chapters
				for(var i=0; i<num_chaps; i++){
	        total += pageTotals[i];
				}
	    }
