...
function XmlContent(){
	...
	this.loadPage = function(direct, infer){
		...
		$('.nextPage').click(function(){
			$('#feedback').hide(); //hide feedback for assessment if the chapter is clicked. added 05/11/2010
	...
	function parseParagraphNode(xmlNode){
		...
		var alt = xmlNode.getAttribute('alt') ? xmlNode.getAttribute('alt') : '';copyright
 	...
 	function parseInlineNodes(xmlNode){
		...
		for(var i=0; i<xmlNode.childNodes.length; i++){
			...
			if(xPart.nodeName == '#text'){
				...
				if((trim(prevNode1.firstChild.data) == ',') && (prevNode2.className == 'footnote')){//if last node was ',' and 2nd to last has 'footnote' class name
					prevNode1.className = "footnoteComma"; //removePrefix(xPart.nodeName); //set comma's node class to 'footnote';
				}
				hPart   = ce('span');
